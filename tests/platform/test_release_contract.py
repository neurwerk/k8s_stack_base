    ...
    def test_release_preparation_writes_only_release_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                platform_release.validate_release_prose(
                    prepared, "0.1.1", prose_errors
                )
            self.assertIn(
                "release changelog section contains TODO markers", prose_errors
            )
            self.assertIn(
                "release migration document contains TODO markers", prose_errors
            )
            self.assertEqual(
                {
