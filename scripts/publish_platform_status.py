...
def publish_status(
    *,
    ...
    """Publish to the calling client's repository, never implicitly to Base."""
    ...
    if RUN_ID_PATTERN.fullmatch(run_id) is None or RUN_ID_PATTERN.fullmatch(run_attempt) is None:
        raise StatusError("workflow run ID or attempt is invalid")
    if not token:
    ...
    request = Request(
        f"{api_url}/repos/{quote(owner)}/{quote(name)}/statuses/{sha}",
        data=body,
        headers=headers,
        method="POST",
    )
    try:
        ...
        raise StatusError("GitHub status API request failed; details withheld") from error
...
def main(argv: list[str] | None = None) -> int:
    class Parser(argparse.ArgumentParser):
        def error(self, message):
            self.exit(2, "::error::invalid status CLI arguments; details withheld\n")
